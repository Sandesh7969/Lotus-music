...
export default class OrasDispatcher {
    ...
    constructor(client, guild, channel, player) {
        ...
        this.player.on("start", async () => {
            ...
            if (setting === "oras-old") {
                ...
                let b2 = this.client.utils.button(`custom_id`, `Previous`, 2, `oras_previous`);
                let b3 = this.client.utils.button(`custom_id`, `Pause`, 3, `oras_pause`);
                let b4 = this.client.utils.button(`custom_id`, `Skip`, 2, `oras_skip`);
                let b5 = this.client.utils.button(`custom_id`, `Loop`, 1, `oras_loop`);
                let b6 = this.client.utils.button(`custom_id`, `Backward`, 4, `oras_backward`);
                let b7 = this.client.utils.button(`custom_id`, `VolHigh`, 2, `oras_volHigh`);
                let b8 = this.client.utils.button(`custom_id`, `Shuffle`, 3, `oras_shuffle`);
                let b9 = this.client.utils.button(`custom_id`, `VolLow`, 2, `oras_volLow`);
                let b10 = this.client.utils.button(`custom_id`, `Forward`, 1, `oras_forward`);
                let row = this.client.utils.actionRow([b1, b2, b3, b4, b5]);
                let row1 = this.client.utils.actionRow([b6, b7, b8, b9, b10]);
    ...
    const attachment = new AttachmentBuilder(buffer, { name: `musicard.png` });
            ...
            if (setting === "oras-no") {
                ...
                let opt1 = this.client.utils.menuOption(`Reset Filters`, this.client.emoji.noButtons.filters, `Resets all the filters of the player`, `cypher_filter_reset`);
                let opt2 = this.client.utils.menuOption(`8D`, this.client.emoji.noButtons.filters, `Sets Up 8d filter to the player`, `cypher_filter_8d`);
                let opt3 = this.client.utils.menuOption(`BassBoost`, this.client.emoji.noButtons.filters, `Sets bassboost filter to the player`, `cypher_filter_bassboost`);
                let opt4 = this.client.utils.menuOption(`NightCore`, this.client.emoji.noButtons.filters, `Sets NightCore filter to the player`, `cypher_filter_nightcore`);
                let opt5 = this.client.utils.menuOption(`Vaporwave`, this.client.emoji.noButtons.filters, `Sets Vaporwave filter to the player`, `cypher_filter_vibrato`);
            ...
            if (setting === `oras-special`) {
                let em = this.client.utils
                    ...
                    .setThumbnail(`https://img.youtube.com/vi/${this.current.info.identifier}/maxresdefault.jpg`);
                let b1 = this.client.utils.button(`custom_id`, null, 2, `cypher_previous`, null, this.client.emoji.special.previous);
                let b2 = this.client.utils.button(`custom_id`, null, 2, `cypher_shuffle`, null, this.client.emoji.special.shuffle);
                let b3 = this.client.utils.button(`custom_id`, null, 2, `cypher_pause`, null, this.client.emoji.special.pause);
                let b4 = this.client.utils.button(`custom_id`, null, 2, `cypher_loop`, null, this.client.emoji.special.loop);
                let b5 = this.client.utils.button(`custom_id`, null, 2, `cypher_skip`, null, this.client.emoji.special.skip);
                let b6 = this.client.utils.button(`custom_id`, null, 2, `cypher_volLow`, null, this.client.emoji.special.volLow);
                let b7 = this.client.utils.button(`custom_id`, null, 2, `cypher_backward`, null, this.client.emoji.special.backward);
                let b8 = this.client.utils.button(`custom_id`, null, 2, `cypher_stop`, null, this.client.emoji.special.stop);
                let b9 = this.client.utils.button(`custom_id`, null, 2, `cypher_forward`, null, this.client.emoji.special.forward);
                let b10 = this.client.utils.button(`custom_id`, null, 2, `cypher_volHigh`, null, this.client.emoji.special.volHigh);
            ...
            if (setting === `oras-simple`) {
                let em = this.client.utils
                    ...
                    },
                    {
                        ...
                        value: `${this.current.info.author} ok`,
                        inline: true,
                    },
                ]);
                let opt1 = this.client.utils.menuOption(`Reset Filters`, this.client.emoji.simple.filters, `Resets all the filters of the player`, `cypher_filter_reset`);
                let opt2 = this.client.utils.menuOption(`8D`, this.client.emoji.simple.filters, `Sets Up 8d filter to the player`, `cypher_filter_8d`);
                let opt3 = this.client.utils.menuOption(`BassBoost`, this.client.emoji.simple.filters, `Sets bassboost filter to the player`, `cypher_filter_bassboost`);
                let opt4 = this.client.utils.menuOption(`NightCore`, this.client.emoji.simple.filters, `Sets NightCore filter to the player`, `cypher_filter_nightcore`);
                let opt5 = this.client.utils.menuOption(`Vaporwave`, this.client.emoji.simple.filters, `Sets Vaporwave filter to the player`, `cypher_filter_vibrato`);
                let options = [opt1, opt2, opt3, opt4, opt5];
    ...
    const attachment = new AttachmentBuilder(buffer, { name: `musicard.png` });
            ...
            if (setting === `oras-spotify`) {
                let opt1 = this.client.utils.menuOption(`Reset Filters`, this.client.emoji.spotify.filters, `Resets all the filters of the player`, `cypher_filter_reset`);
                let opt2 = this.client.utils.menuOption(`8D`, this.client.emoji.spotify.filters, `Sets Up 8d filter to the player`, `cypher_filter_8d`);
                let opt3 = this.client.utils.menuOption(`BassBoost`, this.client.emoji.spotify.filters, `Sets bassboost filter to the player`, `cypher_filter_bassboost`);
                let opt4 = this.client.utils.menuOption(`NightCore`, this.client.emoji.spotify.filters, `Sets NightCore filter to the player`, `cypher_filter_nightcore`);
                let opt5 = this.client.utils.menuOption(`Vaporwave`, this.client.emoji.spotify.filters, `Sets Vaporwave filter to the player`, `cypher_filter_vibrato`);
                let options = [opt1, opt2, opt3, opt4, opt5];
                let menu = this.client.utils.menu(`Choose filters`, `help-op`, options);
                let row1 = this.client.utils.actionRow([menu]);
                let b1 = this.client.utils.button(`custom_id`, `Stop`, 4, `oras_stop`);
